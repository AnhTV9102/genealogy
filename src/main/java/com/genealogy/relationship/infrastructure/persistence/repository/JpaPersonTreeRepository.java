 ...
 */
public interface JpaPersonTreeRepository extends JpaRepository<PersonTreeEntity, PersonTreeEntityId> {

    /**
     * Find all ancestors of a person (excluding self), ordered by depth ascending.
     */
    @Query("SELECT pt FROM PersonTreeEntity pt " +
    ...
    @Query("SELECT pt FROM PersonTreeEntity pt " +
           ...
           "ORDER BY pt.depth ASC")
    List<PersonTreeEntity> findAncestorsMaxDepth(@Param("personId") Long personId, @Param("maxDepth") Integer maxDepth);
    ...
    @Query("SELECT pt FROM PersonTreeEntity pt " +
           ...
           "ORDER BY pt.depth ASC")
    List<PersonTreeEntity> findDescendantsMaxDepth(@Param("personId") Long personId, @Param("maxDepth") Integer maxDepth);
}
