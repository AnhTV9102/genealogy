...
public class RelationshipRepositoryImpl implements RelationshipRepository {
    ...
    @Override
    public boolean existsByFromPersonIdAndToPersonIdAndType(Long fromPersonId, Long toPersonId, String type) {
    ...
    public List<Relationship> findParentsByPersonId(Long personId) {
        return jpaRepository.findParentsByPersonId(personId)
                .stream()
                .map(RelationshipPersistenceMapper::toDomain)
                .toList();
    }
    ...
    public List<Relationship> findChildrenByPersonId(Long personId) {
        return jpaRepository.findChildrenByPersonId(personId)
                .stream()
                .map(RelationshipPersistenceMapper::toDomain)
                .toList();
    }
    ...
    public List<Relationship> findSpouseByPersonId(Long personId) {
        return jpaRepository.findSpouseByPersonId(personId)
                .stream()
                .map(RelationshipPersistenceMapper::toDomain)
                .toList();
    }
